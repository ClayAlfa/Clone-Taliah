            ...
            case 'add':
                ...
                if (empty($name) || empty($email) || empty($password) || $role_id <= 0) {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        try {
                            // WARNING: Plain text password (NOT SECURE!)
                            $stmt = $pdo->prepare("
                                INSERT INTO users (name, email, password, role_id, phone, address, created_at) 
                                VALUES (?, ?, ?, ?, ?, ?, NOW())
                            ");
                            $stmt->execute([$name, $email, $password, $role_id, $phone, $address]);
            ...
            case 'edit':
                ...
                if (empty($name) || empty($email) || $role_id <= 0) {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        try {
                            if (!empty($password)) {
                                // Update with new password - WARNING: Plain text password (NOT SECURE!)
                                $stmt = $pdo->prepare("
                                    UPDATE users 
                                    SET name = ?, email = ?, password = ?, role_id = ?, phone = ?, address = ?, updated_at = NOW()
                                    WHERE id = ?
                                ");
                                $stmt->execute([$name, $email, $password, $role_id, $phone, $address, $id]);
                            } else {
